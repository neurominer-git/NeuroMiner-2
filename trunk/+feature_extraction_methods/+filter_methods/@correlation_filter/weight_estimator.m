...
if nargin==2
    ...
    elseif numel(data_obj)~=1
        ...
        dummy_correlation_filter=filter_methods.correlation_filter;
    ...
    filt_obj.model.weight_vector=arrayfun(@(j) abs(corr(data_obj.data(:,j),target)),1:size(data_obj.data,2));
else
