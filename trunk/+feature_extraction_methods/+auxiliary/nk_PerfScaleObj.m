...
% overmatflag is true then the functon wil comp�ute min and max over the
...
if eIN || ~isfield(IN,'zerooutflag')    || isempty(IN.zerooutflag), IN.zerooutflag = true;  end

[mY,nY] = size(Y);

% Check for Minimum
if eIN || ~isfield(IN,'minY') || isempty(IN.minY)
    if IN.AcMatFl, IN.minY = repmat(min(Y(:)),1,nY); else IN.minY = nanmin(Y); end
...
if eIN || ~isfield(IN,'maxY') || isempty(IN.maxY)
    if IN.AcMatFl, IN.maxY = repmat(max(Y(:)),1,nY); else IN.maxY = nanmax(Y); end
...
if IN.zerooutflag
   if ~isfield(IN,'indnonfin')
        IN.indnonfin = (sum(~isfinite(sY))./mY)==1;
   end
