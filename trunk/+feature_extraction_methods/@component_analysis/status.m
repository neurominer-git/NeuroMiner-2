...
%STATUS (COMP_METHOD class) determines the status of a
%COMP_METHOD (TFE) object or array of FE objects.
...
%   describing the status of the correspendent COMP_METHOD 
%   class object.
%   Possible values are:
%
%       1 - 'estimation_ready' - for COMP_METHOD class 
%       objects have no model defined and an empty report property.
%       2 - 'application_ready' - for COMP_METHOD class 
%       objects with defined models and report property valid (success).
%       3 - 'invalid' - any other case.


%   RES=STATUS (R)
%   See also COMP_METHOD, APPLY

%   STATUS (COMP_METHOD class)  revision history:
...
for i=1:numel(r)
    ...
    %% Act: Get the variables to decide between the three status
        %1st - model
        aux_model=isempty(aux.model.component_weight);
        aux_modeltrans=isempty(aux.model.transformation);
        aux_model=aux_model&aux_modeltrans;
        %2nd - reports
