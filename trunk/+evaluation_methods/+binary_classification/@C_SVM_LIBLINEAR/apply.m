...
if nargin==2
    ...
    if ~isa(teval_obj,'evaluation_methods.binary_classification.C_SVM_LIBLINEAR')
    ...
    elseif numel(features)~=1
        ...
        %experiments
        if ~strcmp(features.evaluation_type,'binary_classification')
        ...
        if teval_obj.parameters.param_eval.value{1}
            dummy_evaluation=evaluation_methods.binary_classification.C_SVM_LIBLINEAR;
            try
...
%% Act: Proceeding to the feature estimation/application process
    ...
    switch status_eval
        case 'estimation_ready'
            ...
            %data_class for features and template_evaluation
            %class for models.
            teval_obj=binary_classification.C_SVM_LIBLINEAR(teval_obj.parameters,model,rep_esti);
        case 'application_ready'
            rep_esti=report('C_SVM_LIBLINEAR',report(),true,[features.descriptor ' | application']);
            %Organization of the results of processing in the classes
            %data_class for features and template_evaluation
            %class for models.
            teval_obj=binary_classification.C_SVM_LIBLINEAR(teval_obj.parameters,teval_obj.model,[teval_obj.reports;rep_esti]);
    end
else
