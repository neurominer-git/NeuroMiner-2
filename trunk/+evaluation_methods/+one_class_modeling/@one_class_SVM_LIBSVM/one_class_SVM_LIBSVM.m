classdef one_class_SVM_LIBSVM
    %Summary of one_class_LIBSVM:
    %   The one_class_LIBSVM class integrates the one clas SVM
    %   implementation LIBSVM in NM2 for more info consult
    %   www.csie.ntu.edu.tw/~cjlin/libsvm.
    %
    %   one_class_LIBSVM properties:
    %   parameters - %FORMAT: struct; Description : Structure with the
    %   parameters to be used by one_class_LIBSVM. Each field in the
    ...
    %      {0,1,2,3} :
    %      Description: Kernel to be used in the oc_svm
    %      0 -- linear: u''*v
    ...
    %      oc_svm (only valid if kernel_type value is 2.
    %      Default value - 3;
    ...
    %      Description: set the parameter nu of one class SVM
    %      Default value - 0.5.
    ...
    %      Default value - True.
    %    - weight_data_instances :data_type 'double' : dimension [1 number of examples/instances] : range [0 Inf]
    ...
    %      Default value - True.
    %
    %   If no parameters are provided the one_class_LIBSVM object
    ...
    %   method defined in the one_class_LIBSVM class. If empty
    ...
    %
    %       3 - one_class_LIBSVM(parameters,model,report)
    %
    %       The complete model is provided, the status of this object is
    %       'application_ready'.
    %
    %
    %   one_class_LIBSVM revision history:
    ...
    methods
        function obj =one_class_SVM_LIBSVM(paramet,model,reports)
            ...
            if nargin==0
               ...
               %kernel_type : kernel to be used on oc_svm
               ...
               %degree : of the polynomial kernel to be used by the oc_svm
               ...
               %gamma : in kernel function to be used by the oc_svm
               ...
               %coef0 : set coef0 in kernel function (default 0) to be used by the oc_svm
               ...
               paramet=struct('kernel_type',kernel_type,'degree',degree,'gamma',gamma,'coef0',coef0,'nu',nu,'cachesize',cachesize,'epsilon',epsilon,'shrinking',shrinking,'weight_data_instances',weight_data_instances,'train_eval',train_eval,'param_eval',param_eval);
               obj.parameters=paramet;
               obj.model=struct([]);
               obj.reports=report;
            else
                if (nargin==1)
                    obj=one_class_modeling.one_class_SVM_LIBSVM();
                    if isstruct(paramet)
                        ...
                        if reporta.flag
                            ...
                            error(['one_class_LIBSVM:Class_error','Error using one_class_LIBSVM \n Invalid parameters provided. \n ' reporta.descriptor]);
                        end
                    else
                        error(['one_class_LIBSVM:Class_error','Error using one_class_LIBSVM \n First input (parameters) must be a structure.']);
                    end
                elseif (nargin==3)
                    obj=one_class_modeling.one_class_SVM_LIBSVM(paramet);
                    if ~isstruct(model)
                        error('one_class_LIBSVM:Incompability_error','Error using one_class_LIBSVM \nSecond input (model) must be a structure.')
                    end
                    ...
                    error('one_class_LIBSVM:Invalid_number_of_arguments','Invalid numer of arguments specified please read the one_class_LIBSVM class documentation.')
                end
            end
        end
    end
