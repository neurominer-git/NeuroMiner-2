classdef nu_SVR_LIBSVM
    %   Summary of nu_SVR_LIBSVM:
    %   The nu_SVR_LIBSVM class integrates the epsilon SVR implementation
    %   of LIBSVM in NM2 for more info consult www.csie.ntu.edu.tw/~cjlin/libsvm.
    %
    %   nu_SVR_LIBSVM properties:
    %   parameters - %FORMAT: struct; Description : Structure with the
    %   parameters to be used by nu_SVR_LIBSVM. Each field in the
    ...
    %      Description: set the parameter nu of nu SVR
    %      Default value - 0.5.
    ...
    %      Default value - True.
    %
    %   If no parameters are provided the the nu_SVR_LIBSVM object
    ...
    %   method defined in the nu_SVR_LIBSVM class. If empty
    ...
    %
    %       3 - nu_SVR_LIBSVM(parameters,model,report)
    %
    %       The complete model is provided, the status of this object is
    %       'application_ready'.
    %
    %
    %   nu_SVR_LIBSVM revision history:
    ...
    methods
        function obj =nu_SVR_LIBSVM(paramet,model,reports)
            ...
            if nargin==0
               ...
               paramet=struct('kernel_type',kernel_type,'degree',degree,'gamma',gamma,'coef0',coef0,'cost',cost,'nu',nu,'cachesize',cachesize,'epsilon',epsilon,'shrinking',shrinking,'weight_data_instances',weight_data_instances,'train_eval',train_eval,'param_eval',param_eval);
               obj.parameters=paramet;
               obj.model=struct([]);
               obj.reports=report;
            else
                if (nargin==1)
                    obj=regression.nu_SVR_LIBSVM();
                    if isstruct(paramet)
                        ...
                        if reporta.flag
                            ...
                            error(['nu_SVR_LIBSVM:Class_error','Error using nu_SVR_LIBSVM \n Invalid parameters provided. \n ' reporta.descriptor]);
                        end
                    else
                        error(['nu_SVR_LIBSVM:Class_error','Error using nu_SVR_LIBSVM \n First input (parameters) must be a structure.']);
                    end
                elseif (nargin==3)
                    obj=regression.nu_SVR_LIBSVM(paramet);
                    if ~isstruct(model)
                        error('nu_SVR_LIBSVM:Incompability_error','Error using nu_SVR_LIBSVM \nSecond input (model) must be a structure.')
                    end
                    ...
                    error('nu_SVR_LIBSVM:Invalid_number_of_arguments','Invalid numer of arguments specified please read the nu_SVR_LIBSVM class documentation.')
                end
            end
        end
    end
