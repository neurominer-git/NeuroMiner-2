...
%APPLY (nu_SVR_LIBSVM class) applies the method defined by
%   nu_SVR_LIBSVM class.
...
%   on the nu_SVR_LIBSVM method and producdes a nu_SVR_LIBSVM
...
if nargin==2
    ...
    if ~isa(teval_obj,'evaluation_methods.regression.nu_SVR_LIBSVM')
        error('apply:InputError',['Undefined function '' apply (nu_SVR_LIBSVM class) '' for the input argument of type ''' class(teval_obj) ''' (First input argument must be a nu_SVR_LIBSVM class object).']);
    elseif numel(teval_obj)~=1
        error('apply:InputError',['Invalid number of elements (' num2str(numel(teval_obj)) ') for the first input of function '' apply (nu_SVR_LIBSVM class).']);
    elseif ~isdata_class(features)
        error('apply:InputError',['Undefined function '' apply (nu_SVR_LIBSVM class) '' for the input argument of type ''' class(features) ''' (Second input argument must be a data_class class object).']);
    elseif numel(features)~=1
        error('apply:InputError',['Invalid number of elements (' num2str(numel(features)) ') for the  second input of function '' apply (nu_SVR_LIBSVM class).']);
    else
        %check labels compatibility with this method - uni dimensional classification
        %experiments
        if ~(strcmp(features.evaluation_type,'regression'))
            error('apply:InputError',['Undefined function '' apply (nu_SVR_LIBSVM class) '' for data_class objects with the ''evaluation_type'' property of type ''' features.evaluation_type ''' .']);
        end
        if teval_obj.parameters.param_eval.value{1}
            dummy_evaluation=evaluation_methods.regression.nu_SVR_LIBSVM;
            try
            ...
            if ~param_report.flag
                ...
                error('apply:IncompatibilityError','Incompatible parameters provided to '' apply (nu_SVR_LIBSVM class) ''. Please check the inputs more details can be found in the error_log');
            end
...
%% Act: Proceeding to the feature estimation/application process
    ...
    switch status_eval %selecting the functioning mode
        case 'estimation_ready'
            ...
            option_string=['-q -s 4 ' param2optstring(teval_obj.parameters)];
            ...
            if isempty(model)
               error('apply:FunctionError','It was impossible to estimate a model based on '' nu_SVR_LIBSVM  ''. Please check your screen for more details.');
            end
...
%             end
            ...
            outputs=results(num2cell(output_hard),output_soft,features.dbcode,features.target_values,features.descriptor,class(teval_obj),features.evaluation_type,features.classes);
        otherwise
            error('apply:FunctionError',['Undefined function '' apply (nu_SVR_LIBSVM class) '' for the ' status_eval ' input argument of type ''' class(teval_obj) '''.']);
    end
    ...
    switch status_eval
        case 'estimation_ready'
            rep_esti=report('nu_SVR_LIBSVM',report(),true,[features.descriptor ' | estimation']);
            %Organization of the results of processing in the classes
            %data_class for features and template_evaluation
            %class for models.
            teval_obj=regression.nu_SVR_LIBSVM(teval_obj.parameters,model,rep_esti);
        case 'application_ready'
            rep_esti=report('nu_SVR_LIBSVM',report(),true,[features.descriptor ' | application']);
            %Organization of the results of processing in the classes
            %data_class for features and template_evaluation
            %class for models.
            teval_obj=regression.nu_SVR_LIBSVM(teval_obj.parameters,teval_obj.model,[teval_obj.reports;rep_esti]);
    end
else
    error('apply:InputError','Invalid number of arguments for function '' apply (nu_SVR_LIBSVM class). (number of arguments is not 2)');
end
