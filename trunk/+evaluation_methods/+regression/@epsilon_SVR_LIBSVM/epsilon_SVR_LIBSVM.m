classdef epsilon_SVR_LIBSVM
    %   Summary of epsilon_SVR_LIBSVM:
    %   The epsilon_SVR_LIBSVM class integrates the epsilon SVR
    %   implementation of LIBSVM in NM2 for more info consult
    %   www.csie.ntu.edu.tw/~cjlin/libsvm.
    %
    %   epsilon_SVR_LIBSVM properties:
    %   parameters - %FORMAT: struct; Description : Structure with the
    %   parameters to be used by epsilon_SVR_LIBSVM. Each field in the
    ...
    %     {0,1,2,3} :
    %     Description: Kernel to be used in the epsilon-SVR
    %       0 -- linear: u''*v
    ...
    %       epsilon_SVR (only valid if kernel_type value is 2.
    %       Default value : 3;
    ...
    %      Default value - True.
    %
    %   If no parameters are provided the the epsilon_SVR_LIBSVM object
    ...
    %   method defined in the epsilon_SVR_LIBSVM class. If empty
    ...
    %
    %       2 - epsilon_SVR_LIBSVM(parameters)
    %
    ...
    %
    %       3 - epsilon_SVR_LIBSVM(parameters,model,report)
    %
    %       The complete model is provided, the status of this object is
    %       'application_ready'.
    %
    %
    %   epsilon_SVR_LIBSVM revision history:
    ...
    methods
        function obj =epsilon_SVR_LIBSVM(paramet,model,reports)
            ...
            if nargin==0
               ...
               %kernel_type : kernel to be used on epsilon_SVR
               ...
               %degree : of the polynomial kernel to be used by the epsilon_SVR
               ...
               %gamma : in kernel function to be used by the epsilon_SVR
               ...
               %coef0 : set coef0 in kernel function (default 0) to be used by the epsilon_SVR
               ...
               paramet=struct('kernel_type',kernel_type,'degree',degree,'gamma',gamma,'coef0',coef0,'cost',cost,'epsilon_svr',epsilon_svr,'cachesize',cachesize,'epsilon',epsilon,'shrinking',shrinking,'weight_data_instances',weight_data_instances,'train_eval',train_eval,'param_eval',param_eval);
               obj.parameters=paramet;
               obj.model=struct([]);
               obj.reports=report;
            else
                if (nargin==1)
                    obj=regression.epsilon_SVR_LIBSVM();
                    if isstruct(paramet)
                        ...
                        if reporta.flag
                            ...
                            error(['epsilon_SVR_LIBSVM:Class_error','Error using epsilon_SVR_LIBSVM \n Invalid parameters provided. \n ' reporta.descriptor]);
                        end
                    else
                        error(['epsilon_SVR_LIBSVM:Class_error','Error using epsilon_SVR_LIBSVM \n First input (parameters) must be a structure.']);
                    end
                elseif (nargin==3)
                    obj=regression.epsilon_SVR_LIBSVM(paramet);
                    if ~isstruct(model)
                        error('epsilon_SVR_LIBSVM:Incompability_error','Error using epsilon_SVR_LIBSVM \nSecond input (model) must be a structure.')
                    end
                    ...
                    error('epsilon_SVR_LIBSVM:Invalid_number_of_arguments','Invalid numer of arguments specified please read the epsilon_SVR_LIBSVM class documentation.')
                end
            end
        end
    end
