...
%APPLY (C_SVM_LIBSVM class) applies the method defined by
%   C_SVM_LIBSVM class.
...
%   on the C_SVM_LIBSVM method and producdes a C_SVM_LIBSVM
...
if nargin==2
    ...
    if ~isa(teval_obj,'evaluation_methods.binary_classification.C_SVM_LIBSVM')
        error('apply:InputError',['Undefined function '' apply (C_SVM_LIBSVM class) '' for the input argument of type ''' class(teval_obj) ''' (First input argument must be a C_SVM_LIBSVM class object).']);
    elseif numel(teval_obj)~=1
        error('apply:InputError',['Invalid number of elements (' num2str(numel(teval_obj)) ') for the first input of function '' apply (C_SVM_LIBSVM class).']);
    elseif ~isdata_class(features)
        error('apply:InputError',['Undefined function '' apply (C_SVM_LIBSVM class) '' for the input argument of type ''' class(features) ''' (Second input argument must be a data_class class object).']);
    elseif numel(features)~=1
        error('apply:InputError',['Invalid number of elements (' num2str(numel(features)) ') for the  second input of function '' apply (C_SVM_LIBSVM class).']);
    else
        ...
        if ~(strcmp(features.evaluation_type,'binary_classification')||strcmp(features.evaluation_type,'multiclass_classification'))
            error('apply:InputError',['Undefined function '' apply (C_SVM_LIBSVM class) '' for data_class objects with the ''evaluation_type'' property of type ''' features.evaluation_type ''' .']);
        end
        if teval_obj.parameters.param_eval.value{1}
            dummy_evaluation=evaluation_methods.binary_classification.C_SVM_LIBSVM;
            try
            ...
            if ~param_report.flag
                ...
                error('apply:IncompatibilityError','Incompatible parameters provided to '' apply (C_SVM_LIBSVM class) ''. Please check the inputs more details can be found in the error_log');
            end
...
%% Act: Proceeding to the feature estimation/application process
    ...
    switch status_eval %selecting the functioning mode
        case 'estimation_ready'
            option_string=['-q -s 0' param2optstring(teval_obj.parameters,features.classes)];
            weight_data_instances=teval_obj.parameters.weight_data_instances.value{1};
            try model=evaluation_methods.auxiliary.libsvm3_12.matlab.svmtrain312(weight_data_instances,cell2mat(features.target_values),features.data,option_string);
            catch err
                rethrow(err);
            end
            if isempty(model)
                error('apply:FunctionError','It was impossible to estimate a model based on '' C_SVM_LIBSVM  ''. Please check your screen for more details.');
            end
        ...
        case 'application_ready'
            ...
            outputs=results(num2cell(output_hard),output_soft,features.dbcode,features.target_values,features.descriptor,class(teval_obj),features.evaluation_type,features.classes);
        otherwise
            error('apply:FunctionError',['Undefined function '' apply (C_SVM_LIBSVM class) '' for the ' status_eval ' input argument of type ''' class(teval_obj) '''.']);
    end
    ...
    switch status_eval
        case 'estimation_ready'
            rep_esti=report('C_SVM_LIBSVM',report(),true,[features.descriptor ' | estimation']);
            %Organization of the results of processing in the classes
            %data_class for features and template_evaluation
            %class for models.
            teval_obj=binary_classification.C_SVM_LIBSVM(teval_obj.parameters,model,rep_esti);
        case 'application_ready'
            rep_esti=report('C_SVM_LIBSVM',report(),true,[features.descriptor ' | application']);
            %Organization of the results of processing in the classes
            %data_class for features and template_evaluation
            %class for models.
            teval_obj=binary_classification.C_SVM_LIBSVM(teval_obj.parameters,teval_obj.model,[teval_obj.reports;rep_esti]);
    end
else
    error('apply:InputError','Invalid number of arguments for function '' apply (C_SVM_LIBSVM class). (number of arguments is not 2)');
end
