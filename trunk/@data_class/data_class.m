classdef data_class
    ...
    %       data - FORMAT: 2D numerical matrix;DESCRIPTION: Core
    %       data to be analyzed by NM with rows representing examples 
        ...
        function obj = data_class(data,type,dbcode,covariates,target_values,eval_type,features_descrip,descriptor,feat_group)
            if (nargin>0)
                if (nargin==6)
                    ...
                    if ismatrix(data)
                        if isnumeric(data)
                           ...
                           error('data_class:Class_error','Data property is invalid, invalid data type please read DATA_CLASS class documentation'); 
                        end
                    else
                        error('data_class:Class_error','Data property is invalid, invalid format please read DATA_CLASS class documentation');
                    end
                    %% Type property
                    if ischar(type)
                        obj.type = type;
                    ...
                    if iscell(target_values)
                        ...
                        if numel(aux_type)==1
                            if islogical(target_values{1})
                               target_values=cellfun(@(x) x*1,target_values,'UniformOutput',false);
                            end
                ...
                elseif (nargin==7)
                    ...
                    if ~isempty(features_descrip)
                        if iscellstr(features_descrip)
                            if size(features_descrip,2)==size(data,2)
                            ...
                            elseif size(features_descrip,1)==size(data,2)
                                features_descrip=features_descrip';
                                empty_value=cellfun(@isempty,features_descrip);
                                if ~any(empty_value)
                                    obj.features_descriptor=features_descrip;
                                else
                                ...
                                error('data_class:Class_error',['"features_descriptor " property is invalid the number of features (' num2str(size(data,2)) ') and the number of features descriptors (' num2str(size(features_descrip,1)) ') differ.']);
                            end
                            obj.features_descriptor = features_descrip;
                        else
                ...
                elseif (nargin==9)
                    ...
                    %% Descriptor property
                    if isa(feat_group,'int16')||isa(feat_group,'double')
