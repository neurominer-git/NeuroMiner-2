...
%     end
    ...
    if ~isempty(dbcodes_shared)
        ...
        for i=1:numel(dbcodes_shared)
            ...
            ref_val=aux_vals(1);
            for j=1:numel(aux_vals)
                aux_decision(j)=isequal(aux_vals(j),ref_val);
            end
            decision(i)=numel(unique(aux_decision));
        end
    ...
    if ~isempty(dbcodes_shared)
        if flag_final_covariates
            ...
            for i=1:numel(dbcodes_shared)
                ...
                for j=1:numel(data)
                    data_db=strcmp(data(j).dbcode,aux_val);
