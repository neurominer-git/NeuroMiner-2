...
%   - evaluation_type - evaluation type to which FPR can be applied in
...
%   FPR (RESULTS class)  revision history:
...
for i=1:numel(r)
    ...
    if ~any(isnan(aux{i}))
        ...
        res(i)=NaN;
        warning('FPR:IncompatibilityError',['FPR (RESULTS class) is undifined for empty objects and ' aux.evaluation_type ' results.'])
    end
end
else
