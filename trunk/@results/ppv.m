function res =ppv(r)
%PPV (RESULTS class) determines the PPV (Positive Predictive Value) 
%   of one or more evaluation experiments results from an array of RESULTS 
%   classs objects. RES=PPV(R) computes the MCC of an array of RESULTS 
...
%   represents the PPV of the correspondent component of R 
...
%   - evaluation_type - evaluation type to which PPV can be applied in
...
%   PPV (RESULTS class)  revision history:
...
for i=1:numel(r)
    ...
    if any(strcmp(r(i).evaluation_type,{'binary_classification','semisupervised_learning','one_class_modeling'}))
        if numel(r(i).classes)==2
            ...
            res(i)=NaN;
             warning('PPV:IncompatibilityError','PPV (RESULTS class) is undifined for empty objects and multiclass_classification evaluation types.')
        end
    else
        res(i)=NaN;
        warning('PPV:IncompatibilityError',['PPV (RESULTS class) is undifined for empty objects and ' r(i).evaluation_type ' results.'])
    end
end
else
