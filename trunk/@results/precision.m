function res =precision(r)
%PRECISION(RESULTS class) determines the precision measure of one or more evaluation
%experiments results from an array of RESULTS classs objects.
%
%   RES=PRECISION(R) computes the precision measure of an array of RESULTS class
%   objects (R) based on the CONFUSION_MATRIX of each one of them. 
%   RES is a cell of the same size as R, each position of RES
%   represents the precision measure of the correspondent element of R 
%   for binary classification classifical precision is estimated while in 
%   the multi class case is estimated the macro averaging of the individual 
%   class precision. Evaluation processes that do not correspond to 
%   evaluation are treated as NaN.
%
...
%   - evaluation_type - evaluation type to which PRECISION can be applied in
...
%   PRECISION(RESULTS class)  revision history:
...
for i=1:numel(r)
    %% Act: Computing the precision measure
    if ~any(strcmp(r(i).evaluation_type,{'binary_classification','multiclass_classification','semisupervised_learning','one_class_modeling'}))
        aux=diag(confusion{i})./((sum(confusion{i},1)+eps)');
        if numel(aux)==2
            res(i)=aux(1);
        else
            res(i)=mean(aux);
        ...
        res(i)=NaN;
        warning('precision:IncompatibilityError',['precision (RESULTS class) is undifined for empty objects and ' r(i).evaluation_type ' results.'])
    end
end
else
