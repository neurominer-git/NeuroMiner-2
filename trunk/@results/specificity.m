function res =specificity(r)
%SPECIFICITY (RESULTS class) determines the specificity of one or more evaluation
%experiments results from an array of RESULTS classs objects.
%   RES=SPECIFICITY (R) computes the specificity of an array of RESULTS class
%   objects (R) based on the CONFUSION_MATRIX of each one of them. 
%
%   RES is a vector of the same size as R, each position of RES
%   represents the specificity of the correspondent component of R 
...
%   - evaluation_type - evaluation type to which SPECIFICITY can be applied in
...
%   SPECIFICITY (RESULTS class)  revision history:
...
for i=1:numel(r)
    %% Act: Computing the specificity
    if any(strcmp(r(i).evaluation_type,{'binary_classification','semisupervised_learning','one_class_modeling'}))
        if numel(aux{i})==2
            res(i)=aux{i}(2);
        else
            res(i)=NaN;
            warning('specificity:IncompatibilityError','specificity (RESULTS class) is undifined for empty objects and multiclass_classification evaluation types.')
        end
    else
        res(i)=NaN;
        warning('specificity:IncompatibilityError',['specificity (RESULTS class) is undifined for empty objects and ' r(i).evaluation_type ' results.'])
    end
end
else
