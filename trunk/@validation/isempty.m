...
%ISEMPTY is True for empty validation class objects or arrays, FALSE
%otherwise.
%  RES=ISEMPTY (R) when R is a single validation object, r is a
...
%
%   See also VALIDATION, ISVALIDATION.

%   ISEMPTY (validation class)  revision history:
%   Date of creation: 8 July 2014 beta (Helena)
%   Creator: Carlos Cabral
res=zeros(1,numel(r));
for i=1:numel(r)
    aux=r(i);
    if isempty(aux.method)&&isempty(aux.design)&&isempty(aux.reports)
        res(i)=1;
    end
end
res=res>0;
