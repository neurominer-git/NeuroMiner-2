...
if nargin==2||nargin==1
    ...
    if ~isempty(parameter_ref)
        ...
        if isparameters(parameter_ref)
            %check values
            if ~isempty(strfind(parameter_ref.comment,'precommand'))
               aux_ini=strfind(parameter_ref.comment,'precommand');
               aux_comment=parameter_ref.comment(aux_ini(1):end);
               aux_ini=find(aux_comment==':')+1;
               aux_final=find(aux_comment=='|')-1;
               eval(aux_comment(aux_ini:aux_final));
            end
