...
if nargin==3||nargin==2
    ...
    if ~(isempty(parameter_ref)&&isempty(parameter))
        ...
        if isstruct(parameter_ref) %if the parameter_ref variable is a structure parameter also needs to be a structure
            ...
            res_fields_flag=isfield(parameter,fieldnames(parameter_ref));
            if ~all(res_fields_flag)
                error('parameters_cheker:Function_error','Error in function '' PARAMETERS_CHECKER '' not all necessary parameters were provided.')
            end
        end
        %% Act
        ...
        if isparameters(parameter_ref)
            ...
            if ~flag_empty
                ...
                elseif ischar(parameter.value{1})
                    if ~isempty(strfind(parameter_ref.comment,'range not applicable'));
                       flag_range=true;
                    else
                        for i=1:numel(parameter.value)
