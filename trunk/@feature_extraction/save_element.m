...
%SAVE_ELEMENT (FEATURE_EXTRACTION class) save to the disc the 
%FEATURE_EXTRACTION elements R.
% RPT=SAVE_ELEMENT (R,FILENAME,LOCATION,FLAG) save the feature_extraction class
...
%
%   FLAG defined the saving mode, if TRUE the program will save the 
%   'economy' version of the feature_extraction model. If FLAG is FALSE the 
%   complete model is saved.
...
%   SAVE_ELEMENT (R)
%   See also FEATURE_EXTRACTION, APPLY, STATUS

%   SAVE_ELEMENT (FEATURE_EXTRACTION class)  revision history:
...
if nargin==4
    if ~(iscellstr(filename)||ischar(filename))
        error('save_element:InputError',['Undefined function '' save_element (feature_extraction class) '' for the input argument of type ''' class(filename) ''' (Second input argument must be a string or a cell of strings).']);
    elseif ~(iscellstr(location)||ischar(location))
        error('save_element:InputError',['Undefined function '' save_element (feature_extraction class) '' for the input argument of type ''' class(location) ''' (Third input argument must be a string or a cell of strings).']);
    elseif ~(islogical(flag))
        error('save_element:InputError',['Undefined function '' save_element (feature_extraction class) '' for the input argument of type ''' class(flag) ''' (Fourth input argument must be logical).']);
    end
    if iscell(location)
        if numel(location)~=numel(r)
            error('save_element:IncompatibilityError',['Incompatible parameters provided to '' save_element (feature_extraction class) ''. The number of elements to be saved (' num2str(numel(r)) ') is not compatible with the number of FILENAMES provided (' num2str(numel(filenames)) ').']);
        end
        loc_flag=true;
    else
        loc_flag=false;
    ...
    if iscell(filename)
        if numel(filename)~=numel(r)
            error('save_element:IncompatibilityError',['Incompatible parameters provided to '' save_element (feature_extraction class) ''. The number of elements to be saved (' num2str(numel(r)) ') is not compatible with the number of LOCATIONS provided (' num2str(numel(locations)) ').']);
        end
    ...
    if numel(flag)>1
        if numel(flag)~=numel(r)
            error('save_element:IncompatibilityError',['Incompatible parameters provided to '' save_element (feature_extraction class) ''. The number of elements to be saved (' num2str(numel(r)) ') is not compatible with the number of saving methods provided (' num2str(numel(flags)) ').']);
        end
        flag_flag=true;
    else
        flag_flag=false;
    ...
    for i=1:numel(r)
        ...
        rpt(i)=report('save_element:feature_extraction',report(),~aux_report,aux_comment);
        
    end
else
    error('save_element:InputError','Invalid number of arguments for function '' save_element (feature_extraction class). (number of arguments is not 4)');
end
end