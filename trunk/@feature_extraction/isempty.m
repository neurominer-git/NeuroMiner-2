...
%ISEMPTY is True for empty FEATURE_EXTRACTION class objects or arrays, FALSE
%otherwise.
%  RES=ISEMPTY (R) when R is a single FEATURE_EXTRACTION object, r is a
...
%
%   See also FEEATURE_EXTRACTION, ISFEATURE_EXTRACTION.

%   ISEMPTY (FEATURE_EXTRACTION class)  revision history:
%   Date of creation: 26 May 2014 beta (Helena)
%   Creator: Carlos Cabral
res=zeros(1,numel(r));
for i=1:numel(r)
    aux=r(i);
    if isempty(aux.models)&&isempty(aux.features)&&isempty(aux.reports)
        res(i)=1;
    end
end
res=res>0;
